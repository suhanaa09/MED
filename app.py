...
- You MUST use the structured tool-calling interface provided. NEVER write tool calls as text like <function=...> or ```tool_call```.
- For ANY question — medical or not — always use wikipedia_live_tool if the topic is not in your local knowledge base.
- For non-medical general knowledge questions (politics, geography, people, events), use wikipedia_live_tool directly.
- Only answer from your own knowledge if NO tool is relevant.

## Tools Available:
1. hybrid_rag_retriever — PRIMARY for medical questions: FAISS semantic + BM25 keyword combined.
2. faiss_rag_retriever — Dense semantic search for conceptual medical questions.
3. bm25_rag_retriever — Keyword search for exact medical terms or drug names.
4. wikipedia_live_tool — Use for ANY topic not covered by local RAG (news, people, events, rare topics).
5. symptom_checker — ALWAYS call when the user mentions any body symptoms.
6. drug_information — ALWAYS call when a specific drug or medication is named.

## Rules:
- Medical question → start with hybrid_rag_retriever, then wikipedia_live_tool if needed.
- General/non-medical question → use wikipedia_live_tool directly.
- Symptoms described → call symptom_checker immediately.
- Drug named → call drug_information immediately.
- Structure answers with headers and bullet points.
- Always end with a Sources & Tools Used section listing each tool invoked.
- Finish every response with the medical disclaimer below.
...
def run_with_fallback(question: str, history: list, tools: list) -> tuple[str, list]:
    """
    Run the LangGraph agent. If Groq returns a malformed tool_use_failed error,
    fall back to a plain LLM call (no tools) so the user always gets an answer.
    ...
    try:
        ...
        return reply, tools_used, ai_msgs[-1] if ai_msgs else AIMessage(content=reply)
    except Exception as e:
        err = str(e)
        # Groq tool_use_failed — retry without tools (plain LLM)
        if "tool_use_failed" in err or "Failed to call a function" in err:
            ...
            plain_msgs = [SystemMessage(content=(
                "You are MediBot, a helpful medical AI assistant. "
                "Answer the user's question as best you can using your own knowledge. "
                "End with: *Medical Disclaimer: For educational purposes only. Always consult a qualified healthcare professional.*"
            ))] + list(history)
            response = llm_plain.invoke(plain_msgs)
...
def build_agent(tools):
    ...
    def call_model(state: AgentState):
        ...
        # Safety: if Groq returned malformed tool_calls, clear them to avoid cascade error
        if hasattr(response, "tool_calls") and response.tool_calls:
            for tc in response.tool_calls:
                if not isinstance(tc.get("name"), str) or not tc.get("name"):
                    response.tool_calls = []
                    break
    ...
    with st.chat_message("assistant"):
        if not os.environ.get("GROQ_API_KEY"):
            ...
            with st.spinner("MediBot is thinking..."):
                try:
                    reply, tools_used, ai_msg = run_with_fallback(
                        question, st.session_state.conversation_history, tools
                    )
                    ...
                    if tools_used:
                        ...
                        st.caption("⚡ Answered directly (no RAG tools needed)")
